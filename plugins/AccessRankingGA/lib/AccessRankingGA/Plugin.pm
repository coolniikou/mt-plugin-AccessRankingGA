...
sub _hdlr_analytic_tags {
	...
	#exclude specific pagepath feauture
...
sub get_token{
	...
	if ($res->is_success) {
		...
		MT->log({
			message => $plugin->translate('as_token_error'). $res->status_line,
			class => 'system',
			level => MT::Log::ERROR(), 
		});
...
sub get_data{
	...
	my $url = "https://www.google.com/analytics/feeds/data?"
		...
		."filters=ga%3ApagePath!%3D%2F&" #;gd%3ApagePath!~css&" ## add filter option, if would like to exclude TOP Page's data 
		."start-date=$start&"
		."end-date=$end&"
	...
	if ($res->is_success) {
		...
		MT->log({
			message => $plugin->translate('as_contents_error'). $res->status_line,
			class => 'system',
			level => MT::Log::ERROR(),
		});
